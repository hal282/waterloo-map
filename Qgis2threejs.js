...
gui.showQueryResult = function (point, layer, obj, show_coords) {
  ...
  const names = layer.properties.propertyNames;
  const hasField = name => names.includes(name);
  const get = name => {
    const i = names.indexOf(name);
    return i !== -1 ? props[i] : "";
  };

  const unitLabel = hasField("Division") ? get("Division") :
                    hasField("Battery") ? get("Battery") : "Unnamed Unit";
  const commander = hasField("Commander") ? get("Commander") : "Unknown";
  const bio = hasField("DivBio") ? get("DivBio") : "No biography available.";
  const corpsHTML = hasField("Corps") ? `
    <div class="info-line"><b>Corps:</b> ${get("Corps")}</div>
  ` : "";

  // Safely get portrait and painting, checking for null/undefined/blank
  const portrait = get("Portrait");
  const painting = get("Paintings") || get("paintings");

  const isValid = url => url && url.trim() !== "" && url.trim().toLowerCase() !== "null" && url.trim().toLowerCase() !== "undefined";

  const hasPortrait = isValid(portrait);
  const hasPainting = isValid(painting);

  let imagesHTML = "";
  if (hasPortrait && hasPainting) {
    imagesHTML = `
      <div class="images-row">
        <img class="portrait" src="${portrait}" alt="Commander Portrait">
        <img class="painting" src="${painting}" alt="Action Painting">
      </div>
    `;
  } else if (hasPortrait) {
    imagesHTML = `
      <div class="images-row single-image">
        <img class="portrait" src="${portrait}" alt="Commander Portrait">
      </div>
    `;
  } else if (hasPainting) {
  ...
  const html = `
    ...
    <div class="popup-container">
      ...
      <div class="division-name">${unitLabel}</div>
      <div class="commander-name">Commander: ${commander}</div>
      ${corpsHTML}
      <div class="biography"><b>Biography:</b> ${bio}</div>
